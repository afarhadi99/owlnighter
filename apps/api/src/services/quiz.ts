...
import { QUIZ_SYSTEM_FRAMING } from "./quiz-prompt.generated.js";
...
// The framing/system text lives in docs/ai-tutor-workflows/build.mjs and is
// emitted here as QUIZ_SYSTEM_FRAMING (imported above) AND baked into the AI
// Tutor API workflow's template, so the two can never silently drift. The
// Gemini/Groq/OpenRouter path composes its own prompt from this framing (as
// `system`) plus userPrompt() (as `user`); the AI Tutor API path ignores both
// and interpolates the discrete `variables` map into its platform-side template.
...
function userPrompt(step: typeof schema.readingPlanSteps.$inferSelect, mode: QuizMode, req: QuizGenerateRequest): string {
  ...
  if (mode === "user_text" && req.userProvidedText) {
    parts.push("", READER_TEXT_INTRO, req.userProvidedText);
  } else if (mode === "fallback") {
    parts.push("", FALLBACK_CONTEXT);
  }
  return parts.join("\n");
}

const READER_TEXT_INTRO = "Reader-supplied page text (base questions strictly on this):";
const FALLBACK_CONTEXT = "No page-level text is available. Write general comprehension prompts about the reading's themes.";

/**
 * The discrete named template variables the AI Tutor API quiz workflow expects.
 * Names + intent must stay in lockstep with the workflow's {{placeholders}} and
 * `inputs` (both generated by docs/ai-tutor-workflows/build.mjs). Derived from
 * the same sources as userPrompt() so the two prompt paths carry identical data;
 * values are always strings (the flat request body has no room for undefined).
 * Exported for services/quiz.test.ts.
 */
export function quizVariables(
  step: typeof schema.readingPlanSteps.$inferSelect,
  mode: QuizMode,
  req: QuizGenerateRequest,
): Record<string, string> {
  const readerContext =
    mode === "user_text" && req.userProvidedText
      ? `${READER_TEXT_INTRO}\n${req.userProvidedText}`
      : mode === "fallback"
        ? FALLBACK_CONTEXT
        : "";
  return {
    stepTitle: step.title,
    chapterHint: step.chapterHint ?? "",
    pageRange: step.pageStart != null && step.pageEnd != null ? `${step.pageStart}-${step.pageEnd}` : "",
    quizMode: mode,
    questionCount: String(req.questionCount),
    readerContext,
  };
}
...
export async function generateStepQuiz(deps: Deps, user: AuthUser, stepId: string, req: QuizGenerateRequest): Promise<QuizInstance> {
  ...
  const system = QUIZ_SYSTEM_FRAMING;
  ...
  try {
    const r = await deps.ai.generateObject<GeneratedQuiz>({
      ...
      // system + user drive the Gemini/Groq/OpenRouter path; variables drive the
      // AI Tutor API path (its templated workflow interpolates the named map and
      // ignores system/user). Both are populated so any provider can serve.
      system,
      user: prompt,
      variables: quizVariables(step, mode, req),
    });
